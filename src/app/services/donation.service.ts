...
export class DonationService {
  ...
  public getDonationQuantityAndType(): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiServerUrl}/product/typeQty`);
  }

  public getDonationById(donationId: number): Observable<Donation> {
    return this.http.get<Donation>(`${this.apiServerUrl}/product/find/${donationId}`);
  }

  public addDonation(donation: Donation): Observable<Donation> {
    return this.http.post<Donation>(`${this.apiServerUrl}/product/add`, donation);
  }

  public deleteDonation(donationId: number): Observable<void> {
    return this.http.delete<void>(`${this.apiServerUrl}/product/delete/${donationId}`);
  }

  public updateDonation(donation: Donation): Observable<Donation> {
    return this.http.put<Donation>(`${this.apiServerUrl}/product/update`, donation);
  }
